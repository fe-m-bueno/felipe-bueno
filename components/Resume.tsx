    ...
    <section className="px-6 py-6 md:py-24 flex flex-col justify-between md:h-screen min-h-screen ">
      ...
      <ul className="my-4 space-y-4">
        {data.experience.map((exp, index) => (
          <li
            key={index}
            className="p-6 border dark:border-white/20 border-black/20 rounded-3xl bg-white/[3%] hover:bg-white/[9%] transition-all ease-in-out duration-200 backdrop-blur-3xl hover:-translate-y-1"
          >
      ...
      <ul className="my-4">
        {data.additionalEducation.map((edu, index) => (
          <li
            key={index}
            className="p-4 border dark:border-white/20 border-black/20 rounded-3xl bg-white/[3%] hover:bg-white/[9%] transition-all ease-in-out duration-200 backdrop-blur-3xl hover:-translate-y-1"
          >
            <h4 className="~text-lg/xl font-bold">{edu.institution}</h4>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {edu.degree} • {edu.date}
