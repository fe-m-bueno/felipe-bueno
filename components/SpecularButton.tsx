...
let isPointerListenerActive = false;

function dispatchPointerMove(event: PointerEvent) {
  pointerSubscribers.forEach((subscriber) => subscriber(event));
}

function subscribeToPointerMove(subscriber: (event: PointerEvent) => void) {
  pointerSubscribers.add(subscriber);
  if (!isPointerListenerActive) {
    window.addEventListener("pointermove", dispatchPointerMove, {
      passive: true,
    });
    isPointerListenerActive = true;
  }

  return () => {
    pointerSubscribers.delete(subscriber);
    if (pointerSubscribers.size === 0 && isPointerListenerActive) {
      window.removeEventListener("pointermove", dispatchPointerMove);
      isPointerListenerActive = false;
    }
  };
}
...
const SpecularButton = ({
  ...
  const setButtonRef = (node: HTMLButtonElement | HTMLAnchorElement | null) => {
    btnRef.current = node;
  };
